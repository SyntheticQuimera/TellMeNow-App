...
async function register(input) {
  ...
  if (foundEmail) throw new Error("The email address is already in use");

  // Revisamos si el username esta en uso
  const foundUsername = await User.findOne({ username });
  if (foundUsername) throw new Error("The username is already in use");
...
async function login(input) {
  ...
  if (!passwordSucess) throw new Error("Your email or password is incorrect");
